...
- (void)handleStreamComplete
{
	...
	if (![self needsRedirect] && ![self authenticationNeeded] && ![self didUseCachedResponse]) {
		...
		if (fileError) {
            ...
            if(self.responseStatusCode > 399) {
                NSDictionary* _userInfo = [NSDictionary dictionaryWithObject:self.responseString forKey:@"responseString"];
                NSError* err = [NSError errorWithDomain:NetworkRequestErrorDomain code:self.responseStatusCode userInfo:_userInfo];
                [self failWithError:err];
            }
            else{
